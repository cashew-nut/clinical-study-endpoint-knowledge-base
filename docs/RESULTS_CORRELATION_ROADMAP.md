...
**Shipped.** The NI margin turned out not to be a column on either source: it
is stated, when at all, in the free-text `non_inferiority_description`, so
`results/effects.py` parses it conservatively and shows the prose whenever it
cannot read a number out of it.
